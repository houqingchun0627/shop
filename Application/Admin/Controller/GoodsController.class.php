 ...
 * @Last Modified time: 2016-08-01 21:57:33
 */
namespace Admin\Controller;

class GoodsController extends \Admin\Controller\BasicController {
    public function test(){
        // $info = D("Goods")->where('goods_id = 3')->setInc('goods_view_amount'); // 用户的积分加1
        $goodsId = 3;
        $info = D("Goods")->where('goods_id = ' .$goodsId)->setInc('goods_view_amount');
        echo "<pre>";
        var_dump($info);
        exit;
    }
            ...
            if(!empty($id)){
                ...
                if((1 == $id) || (2 == $id)){
                    ...
                    $userPrice = D('UserLevel')->alias("a")->field("a.level_id,a.level_name,a.level_discount,b.goods_price,b.id")->join(array(
                        "LEFT JOIN __USER_PRICE__ b ON b.user_level = a.level_id AND b.goods_promote = '否' AND b.goods_id = ".$gid
                    ))->select();
        ...
        if(IS_GET){
            ...
            if(!empty($id)){
                ...
                if((1 == $id) || (2 == $id)){
                    ...
                    //获取当前商品品牌下的所有的商品分类
                    $catList = explode(',',$brandList["cat_id"]);
                    // echo "<pre>";
                    // var_dump($catList);
                    // exit;
                    $this->assign(array(
...
/********************以下是商品列表展示页面ajax操作*******************************/
    ...
    public function ajaxGoodsList(){
        $data = D("Goods")->ajaxSearch(I('post.'),1);
        $this->assign(array(
                // 'goodsList'     =>      $goodsList,
                'goodsList'     =>      $data['list'], 
                'page'          =>      $data['page'],
